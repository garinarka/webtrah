...
onMounted(() => {
    ...
    window.addEventListener('resize', ui.updateIsMobile)
})
onUnmounted(() => {
    window.removeEventListener('resize', ui.updateIsMobile)
})
...
const navigationItems = computed(() => {
    ...
    const items = [
        ...
        { name: 'Keluarga', href: '/family', icon: 'House' },
...
<template>
    <div :class="['min-h-screen bg-gray-50', bodyClasses]">
        ...
        <button
            @click="ui.openSheet('senior-mode')"
            class="fixed right-4 top-20 z-30 rounded-full bg-white p-2 text-xl shadow"
            title="Mode Lansia"
        >
            &#x1F474;
        </button>
        ...
        <div class="flex pt-16">
            ...
            <main
                ...
                :class="{
                    'ml-64': !ui.isMobile && ui.sidebarOpen,
                    'ml-16': !ui.isMobile && !ui.sidebarOpen,
                    'pb-20': ui.isMobile,
                }"
            >
        ...
        <Fab
            v-if="auth.isAdmin || auth.isModerator || auth.isUser"
            :actions="fabActions"
        />

        <!-- bottom sheet -->
        <BottomSheet
            :is-open="!!ui.activeSheet"
            :title="sheetContent?.title"
            @close="ui.closeSheet"
        >
            <component
                :is="sheetContent?.component"
                v-if="sheetContent?.component"
            />
