...
export function usePersonForm(initialData = {}) {
    ...
    const form = useForm({
        ...
        // Relasi awal yang akan dibuat setelah person tersimpan (dari StepFamily)
        initial_relations: initialData.initial_relations ?? [],
    })
    ...
    const validateField = (field, value = form[field]) => {
        ...
        if (field === 'death_date' && value) {
            const parts = value.split('-').map(Number)
            const death = new Date(parts[0], (parts[1] ?? 1) - 1, parts[2] ?? 1)
